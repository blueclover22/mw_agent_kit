...
5. **If no markers, check for legacy content** — earlier versions of this kit were installed by manually pasting sections into `~/.claude/CLAUDE.md`. Look outside any markers for:
   - An `## Agents` / `## Skills` catalog listing planner/coder/reviewer/doc-editor or the 7 kit skills, and a `## Workflow` section matching the kit's task grades: these are superseded by the plugin and this block. **Do not delete anything silently** — show the user what overlaps and propose removing the duplicates before appending the block. Only proceed after confirmation.
   - `## Rules` / `## Coding Rules` / `## File Handling` sections: these are considered the user's personal settings. Leave them untouched; just note they exist. (The block's own coding-principle mapping coexists with a personal `## Coding Rules` — the personal one takes precedence on conflict.)
