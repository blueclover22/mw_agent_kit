...
- Development-process skills/agents come from the `mak` plugin — skills: `mak:brainstorming` (diverge) → `mak:dev-kickoff` (kickoff, approval gate) → `mak:design-doc-template` / `mak:major-feature-pack` (design documentation) → `mak:verify-checklist` (verification) → `mak:review-report` (review) → `mak:doc-audit` (cross-document consistency audit) → `mak:commit` (wrap-up commit — invoking the skill is the explicit commit request; other git ops such as push run only on explicit request); top-level axis `mak:roadmap-planning`. Agents: `mak:planner` / `mak:coder` / `mak:reviewer` / `mak:doc-editor` / `mak:analyzer`. Follow each mak skill/agent description for flow details.
