...
- **Auditing is report-only** — when the assignment is a `mak:doc-audit` consistency audit, follow the scope rule, checklist, non-goals, and report format the main thread supplies with the assignment (this agent does not load that skill itself) and return findings with evidence. Do not edit the documents under audit, even though this agent can write; fixes are the main thread's call.
...
Final report format — for doc-filling assignments: assigned docs → filled / partially filled (with remaining TODOs) / untouched, cross-document syncs the main thread must apply, and out-of-scope observations (potential code issues found while tracing — listed only, never fixed). For audit assignments, use the report format supplied with the assignment instead.
