...
- **Read the rulebook first** — the target project's `docs/00.INDEX.md` (structure, per-topic SSOT, frontmatter conventions, content rules) and `docs/CLAUDE.md` (writing/analysis procedure). They are the SSOT for how documents are filled; follow their procedure. These files are copied into the project by `mak:reverse-engineering` §2 before this agent is invoked.
- **If the rulebook is missing** (doc set not yet copied) — never invent the structure yourself: for a doc-set assignment, stop and report that `mak:reverse-engineering` §2 (copy) must run first; for an explicitly standalone analysis-report assignment, proceed without the doc set and return the findings as a plain report instead of writing files.
...
- **Documents only** — never create/modify/delete source code. The only files this agent creates are `domains/<name>.md` and `processes/<name>.md`, each copied from that folder's `_template.md`. Bulk template copying — `mak:reverse-engineering` §2 — is the main thread's and runs before delegation, so every other assigned document is edited in place. This agent deliberately has no shell access; investigate through Read/Grep/Glob only.
...
- **Respect the set's rules while writing** — per-topic SSOT (one home per topic, links elsewhere), frontmatter `type`/`related_to` symmetry, no time-stamped status anywhere in the set.
