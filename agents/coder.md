...
- **Verify per step** — for multi-step work, verify against the design doc §5 `Step → verify: check` table. Never move to the next step with verification missing.
...
- **Escalate instead of guessing** — if the implementation is hard enough that accuracy/quality feels insufficient on the current model (`sonnet`), do not push through; report it and suggest re-running with a stronger model.
