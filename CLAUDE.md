...
- The research-doc save-path rule has ONE home: `agents/researcher.md` §Save location. The main thread applies it and passes the resolved path in, so the default is restated where the main thread can read it — `skills/setup/assets/claude-md-snippet.ko.md`/`.en.md`. Changing the default means updating both restatements — grep `docs/research/` to find them
...
- The task grades and the four coding principles have ONE home: `docs/guide.md §2.1 / §2.2`. The setup snippets inject copies of both (task-grade table from §2.1, principle mapping from §2.2 — the snippet copy keeps only the first two columns, Principle / Core behaviors, and drops Enforcement gates) + the mak delegation rules — a change to any column the snippet copies must be synced across 4 places: guide ko/en + snippet ko/en; a change confined to the Enforcement gates column syncs guide ko/en only. Skills/agents keep the "project/global Coding Rules take precedence" phrasing
